...
本文提出\textbf{Whisper}——一门旨在最小化Token消耗的栈式后缀编程语言。
\textbf{Whisper}采用单字符操作符（\texttt{\_} 复制、\texttt{`} 交换、
\texttt{@} 旋转）、紧凑的条件语法（\texttt{?? ... | ... ]}）、
22个核心内置词（无需import即可使用）以及基于能力的零信任安全模型。
我们在1,149条\textbf{Whisper}指令-响应对上微调了Qwen2.5-Coder-7B模型。
在15个任务的基准测试上，模型实现了40.0\%的精确匹配准确率和93.3\%的
语法有效性，可执行的生成代码中83.3\%输出了正确结果。
在25个任务的手写代码Token对比中，\textbf{Whisper}比等效Python
少消耗24.0\%的Token，在17/25个任务上更优。
消融实验表明，标准库贡献了6.7个百分点的准确率提升和13.3个百分点的
语法有效性提升，而数据规模在750个样本处达到饱和。
实验结果表明，面向LLM Token经济性的语言级优化可以带来可量化的成本节约。
\end{abstract}
...
我们进一步在\textbf{Whisper} VM上执行生成的代码以验证运行正确性。在
模型生成的6个可运行代码中，5个输出了正确结果，执行通过率（pass@1）为
\textbf{83.3\%}。其余9个任务执行失败的原因是模型生成了\texttt{swap}、
\texttt{over}等当前VM不识别的词——这是编译器实现限制而非模型能力问题。

\subsection{Token效率（RQ2）}

表\ref{tab:token}展示了使用Qwen2.5-Coder分词器计数的手写代码Token对比。在25个
基准任务中，\textbf{Whisper}代码总计使用212个Token，而等效Python代码使用279个
——减少\textbf{24.0\%}。\textbf{Whisper}在17/25个任务（68\%）上更节省Token。

\begin{table}[ht]
\centering
\caption{手写代码Token对比（Qwen2.5-Coder分词器）。}
\label{tab:token}
\begin{tabular}{lrrr}
\toprule
...
abs\_value & 4 & 5 & 20.0\% \\
\midrule
\textbf{总计（25个任务）} & \textbf{212} & \textbf{279} & \textbf{24.0\%} \\
\bottomrule
\end{tabular}
\end{table}

Token节省在算法类任务中最为显著：factorial（70.0\%）、fibonacci（83.3\%）、
is\_prime（67.9\%）、gcd（50.0\%）——这些任务中\textbf{Whisper}的核心内置词
替代了多行递归定义。\textbf{Whisper}在涉及Python字符串语法糖
（\texttt{[::-1]}、\texttt{in}）和Qwen对Python字符串字面量高效分词的任务
上持平或略逊。

\subsection{训练数据规模的影响}

为量化训练数据量与代码生成质量的关系，我们分别使用250、500、750和1,149条
数据训练了四个模型。表\ref{tab:scaling}展示了结果。

\begin{table}[ht]
\centering
\caption{数据规模消融：精确匹配与语法有效性。}
\label{tab:scaling}
\begin{tabular}{lcc}
\toprule
\textbf{训练样本数} & \textbf{精确匹配} & \textbf{语法有效} \\
\midrule
250  & 6.7\%  & 100.0\% \\
500  & 26.7\% & 100.0\% \\
750  & 40.0\% & 93.3\%  \\
...
精确匹配准确率从250到500条急剧上升（6.7\%至26.7\%），继续提升至750条时达到
40.0\%，在1,149条时饱和。语法有效性在低数据量时接近完美，随着模型在更高数据量
下尝试更复杂模式而略微下降。750条处的饱和表明，进一步的准确率提升需要更大的
基座模型或架构改进，而非增加训练数据。

\subsection{标准库的影响（RQ3）}

为量化标准库和核心内置词的贡献，我们使用去除了所有\texttt{import std/*}模式的
数据集（1,097条）训练了一个模型。表\ref{tab:stdlib}对比了两个模型。

\begin{table}[ht]
\centering
\caption{标准库消融：有无标准库模式的影响。}
\label{tab:stdlib}
\begin{tabular}{lcc}
\toprule
\textbf{训练配置} & \textbf{精确匹配} & \textbf{语法有效} \\
\midrule
含标准库+核心内置词（1,149条） & 40.0\% & 93.3\%  \\
无标准库（1,097条）             & 33.3\% & 80.0\%  \\
\midrule
\textbf{差异}                   & \textbf{+6.7\%} & \textbf{+13.3\%} \\
\bottomrule
\end{tabular}
\end{table}

标准库贡献了6.7个百分点的精确匹配提升和13.3个百分点的语法有效性提升。语法有效性
的大幅提升表明，标准库模式作为模板引导模型生成正确的\textbf{Whisper}语法，
减少了不平衡条件和格式错误的定义等错误。

\subsection{成本影响}

按当前API定价（GPT-4o、Claude Opus 4等前沿模型约每百万输出Token \$0.50），
24.0\%的Token减少带来的直接成本节约为：

\begin{itemize}
\item \textbf{每百万次代码生成}：按每次生成平均500个输出Token计算，
  相比Python，\textbf{Whisper}每百万次生成节省约\$60。
\item \textbf{企业规模}：对于每天服务1,000万次AI代码补全的组织，
  年度节省超过\$20万。
\end{itemize}

以上估算基于手写参考代码的输出Token成本，较为保守。实际场景中的节省可能因
生成长度和具体任务分布而有所变化。
...
的Token消耗。\textbf{Whisper}通过单字符操作符、紧凑的条件语法、
22个核心内置词（无需import即可使用）以及基于能力的零信任安全模型实现
Token效率和安全执行。

我们的实证评估表明，微调后的7B参数模型在\textbf{Whisper}代码生成上实现了
40.0\%的精确匹配准确率、93.3\%的语法有效性和83.3\%的执行通过率（pass@1）。
在25个任务的手写代码Token对比中，\textbf{Whisper}比等效Python少消耗24.0\%
的Token，在17/25个任务上更优。消融实验量化了训练数据规模和标准库设计的
贡献。在AI推理成本以每Token计价的时代，语言级优化代表了一个尚未充分开发
的手段，可用于降低AI辅助软件开发的经济和环境成本。
