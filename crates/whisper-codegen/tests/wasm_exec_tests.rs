...
fn extract_bytecode(wasm: &[u8]) -> Vec<u8> {
    ...
    while pos < wasm.len() {
        ...
        if section_id == 11 { // Data section
            ...
            for _ in 0..count {
                assert_eq!(wasm[pos], 0x00); pos += 1; // mode=active, memory=0
                assert_eq!(wasm[pos], 0x41); pos += 1; // i32.const
                let offset = read_leb128_s(wasm, &mut pos) as u32;
                assert_eq!(wasm[pos], 0x0B); pos += 1; // end
...
fn test_wasm_module_structure() {
    ...
    while pos < wasm.len() {
        ...
        if (id as usize) < found.len() { found[id as usize] = true; }
        pos += len;
    }
    for &s in &sections { assert!(found[s as usize], "Missing section {}", s); }
}
...
fn test_wasm_bytecode_roundtrip() {
    ...
    let ops = vec![
        Opcode::PushI64(42), Opcode::PushI64(13), Opcode::Add,
        Opcode::PushI64(55), Opcode::Eq,
    ];
...
fn test_wasm_complex_program_roundtrip() {
    // (10-3)*(2+1) = 21
    let ops = vec![
        Opcode::PushI64(10), Opcode::PushI64(3), Opcode::Sub,
        Opcode::PushI64(2), Opcode::PushI64(1), Opcode::Add,
        Opcode::Mul,
    ];
    let gen = WasmGenerator::new(ops.clone());
    let wasm = gen.compile();
...
fn test_wasm_exports_present() {
    ...
    assert!(wasm_str.contains("whisper_run"), "Missing whisper_run export");
    assert!(wasm_str.contains("memory"), "Missing memory export");
    assert!(wasm_str.contains("get_stack_ptr"), "Missing get_stack_ptr export");
}
