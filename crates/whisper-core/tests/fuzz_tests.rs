...
/// Generate a random i64 for testing.
...
fn random_op(seed: &mut u64) -> Opcode {
    let n = rand_i64(seed).unsigned_abs() % 40;
    match n {
        0 => Opcode::Dup,
        1 => Opcode::Swap,
        2 => Opcode::Drop,
        ...
        32 => Opcode::Each,
        33 => Opcode::Fold,
        34 => Opcode::Cond((rand_i64(seed) % 10 - 5) as i32),
        35 => Opcode::Jump((rand_i64(seed) % 10 - 5) as i32),
        36 => Opcode::Loop((rand_i64(seed) % 10 - 10) as i32),
        37 => Opcode::CapCall((rand_i64(seed) % 5) as u16),
        38 => Opcode::ConfLabel((rand_i64(seed) % 100) as f64 / 100.0),
        _ => Opcode::ProbChoice,
    }
}
...
fn random_program(len: usize, seed: u64) -> Vec<Opcode> {
    let mut s = seed;
    (0..len).map(|_| random_op(&mut s)).collect()
}
...
fn fuzz_stack_underflow() {
    ...
    for op in [Opcode::Add, Opcode::Sub, Opcode::Mul, Opcode::Div,
        ...
        let mut vm = Vm::new();
        assert!(vm.execute(&[op.clone()]).is_err(), "Op {:?} on empty stack should error", op);
    }
}
...
fn fuzz_control_flow() {
    ...
    for mut seed in 0..300 {
        let mut vm = Vm::new();
        vm.data_stack.push(Value::Bool(rand_i64(&mut seed) % 2 == 0));
        let prog = random_program(15, seed + 5000);
        let _ = vm.execute(&prog);
    }
}
